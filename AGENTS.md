...
## Project Overview
`mnt-rms` is a Bun + TypeScript CLI for project-based Remotion rendering. It loads and validates `projects/*/project.ts`, stages assets, then runs `compositions`, `studio`, or `render`. The codebase separates project loading/staging, timeline validation, and composition runtime, writing renders to `output/`.
...
### CLI Command Routing
`src/cli.ts` parses `<action> <project-path>` and dispatches only `compositions | studio | render` to `runVideoCommand`.
...
`src/projects/stage.ts` copies media to `.tmp/remotion-public/<project-id>/...` and prepares `output/<project-id>/...`. `src/commands/write-root-entry.ts` generates the temporary Remotion root entry.
